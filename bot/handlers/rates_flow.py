...
async def rate_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "Выберите исходную валюту:", reply_markup=currency_keyboard()
    )


@router.callback_query(F.data == "cur:cancel")
async def rate_cancel(callback: CallbackQuery, state: FSMContext):
    await state.clear()
...
async def choose_base(callback: CallbackQuery, state: FSMContext):
    ...
    if code == "manual":
        ...
        await callback.message.edit_text("Введите исходную валюту (например: USD):")
        await callback.answer()
        return
    ...
    await callback.message.edit_text(
        f"Исходная валюта: {code}\nТеперь выберите конечную валюту:",
...
async def choose_target(callback: CallbackQuery, state: FSMContext):
    ...
    if code == "manual":
        ...
        await callback.message.edit_text("Введите конечную валюту (например: EUR):")
        await callback.answer()
        return

    if code == base:
        await callback.answer("Нельзя выбрать ту же валюту.", show_alert=True)
        return
...
async def manual_base(message: Message, state: FSMContext):
    data = await state.get_data()
    ...
    if not base.isalpha() or len(base) != 3:
        await message.answer("Введите 3-буквенный код валюты, например: USD")
        return
...
async def manual_target(message: Message, state: FSMContext):
    data = await state.get_data()
    ...
    if not target.isalpha() or len(target) != 3:
        await message.answer("Введите 3-буквенный код валюты, например: EUR")
        return

    if target == base:
        await message.answer("Конечная валюта не должна совпадать с исходной.")
        return
    ...
    async with httpx.AsyncClient() as http:
        ...
        try:
            ...
            await message.answer("⚠️ Сервис курсов валют временно недоступен.")
            await state.clear()
            return
